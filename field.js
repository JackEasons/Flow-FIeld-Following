class field {
  ...
  update (t) {
    ...
    for (let i = 0; i < this.column; i++) {
      ...
      for (let j = 0; j < this.row; j++) {
        const a = map(noise(nx, ny), 0, 1, 0, TWO_PI)
  ...
  caculate (tempx, tempy) {
    const x = floor(constrain(tempx / this.w, 0, this.column - 1))
    const y = floor(constrain(tempy / this.w, 0, this.row - 1))
    return this.field[x][y].copy()
  }

  make2dArray (temp_column, temp_row) {
    const temp_field = new Array(temp_column)
